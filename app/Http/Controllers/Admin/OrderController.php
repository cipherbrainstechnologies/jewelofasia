...
class OrderController extends Controller
{
    ...
    public function list(Request $request, $status): \Illuminate\Contracts\View\Factory|\Illuminate\Contracts\View\View|\Illuminate\Contracts\Foundation\Application
    {
        ...
        if ($status != 'all' && $status !== 'subscription') {
            $query = $this->order->with(['customer', 'branch', 'userSubscription'])
        ...
        foreach ($orders as $order) {
            $user_sub = userSubscription::where('order_id', $order->id)->first();
            $is_sub_porduct = !empty($user_sub) ? 1 : 0;
            $order->is_sub_product = $is_sub_porduct;
        }
        $count_data = [
            ...
            'subscription' => $this->order->with(['userSubscription'])->get()->filter(function ($order) {
