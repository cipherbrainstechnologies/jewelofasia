...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="d-print-none pb-2">
            <div class="row align-items-center">
                <div class="col-auto mb-2 mb-sm-0">
                    <h1 class="page-header-title">{{ translate('customer') }} {{ translate('id') }} #{{ $customer['id'] }}
                    </h1>
                    <span class="d-block">
                        <i class="tio-date-range"></i> {{ translate('joined_at') }} :
                ...
                <div class="col-auto ml-auto">
                    <a class="btn btn-icon btn-sm btn-soft-secondary rounded-circle mr-1"
                        ...
                        data-placement="top" title="{{ translate('Previous customer') }}">
            ...
            <div class="col-lg-6 col-md-6 col-sm-6">
                <div class="resturant-card bg--2">
                    <img class="resturant-icon" src="{{ asset('/public/assets/admin/img/dashboard/1.png') }}"
                        alt="dashboard">
                    <div class="for-card-text font-weight-bold  text-uppercase mb-1">{{ translate('wallet') }}
                        {{ translate('balance') }}</div>
                    <div class="for-card-count">
                        {{ \App\CentralLogics\Helpers::set_symbol($customer->wallet_balance ?? 0) }}
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-6 col-md-6 col-sm-6">
                <div class="resturant-card bg--3">
                    <img class="resturant-icon" src="{{ asset('/public/assets/admin/img/dashboard/3.png') }}"
                        alt="dashboard">
                    <div class="for-card-text font-weight-bold  text-uppercase mb-1">{{ translate('loyalty_point') }}
                        {{ translate('balance') }}</div>
        ...
        <!-- End Row -->
        <div class="row adm-subscription-list" id="printableArea">
            <div class="col-lg-8 mb-3 mb-lg-0">
                <div class="card">
                    <div class="card-header">
                        <div class="card--header">
                            <h5 class="card-title">{{ translate('Subscription List') }} <span
                                    class="badge badge-soft-secondary">{{ count($orders) }}</span></h5>
                            <form action="{{ url()->current() }}" method="GET">
                                <div class="input-group">
                                    <input id="datatableSearch_" type="search" name="search" class="form-control"
                            ...
                            <tbody>
                                @foreach ($subscriptions as $key => $subscription)
                                    <tr>
                                        ...
                                        <td class=" text-center">
                                            <a
                                                href="{{ route('admin.orders.details', ['id' => $subscription['order_id']]) }}">{{ $subscription['order_id'] }}</a>
                                        </td>
                                        <td class="text-center">{{ $subscription['plan'] }}</td>
                                        <td class="text-center">
                                            {{ !empty($subscription['status']) ? 'Active' : 'Canceled' }}</td>
                                        <td>
                                            <!-- Dropdown -->
                                            @if (!empty($subscription['status']))
                                                <div class="btn--container justify-content-center">
                                                    <a class="btn btn-primary" href="javascript:"
                                                        onclick="form_alert('subscription-{{ $subscription['subscription_id'] }}','{{ translate('Want to cancel this subscription ?') }}')">{{ translate('Cancel Subscription') }}</a>
                                                    <form
                                                        action="{{ route('admin.customer.cancel_subscription', [$subscription['subscription_id']]) }}"
                                                        method="get"
                                            ...
                                            @else
                                                <div class="btn--container justify-content-center">
                                                    <button class="btn btn-secondary" disabled>{{ translate('Cancel Subscription') }}</button>
                                                </div>
                                            @endif
                                            <!-- End Dropdown -->
                                        </td>
